...
def test_dict_allclose():
    left = {"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4, 5, 6]), "c": {}}
    right = {"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4, 5, 6]), "c": {}}
    assert dict_cmp.dict_allclose(left, right)


def test_dict_allclose_same_obj():
    dict0 = {"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4, 5, 6]), "c": 1}
    assert dict_cmp.dict_allclose(dict0, dict0)


def test_dict_allclose_different_keys():
    left = {"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4, 5, 6]), "c": 1}
    right = {"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4, 5, 6]), "d": 1}
